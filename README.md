...
- Подмена текста делается через официальный API Dalamud `IAddonLifecycle` (событие `PostDraw`)
  на конкретных нативных окнах (аддонах) — по умолчанию только `ActionDetail`: проверено на живом
  клиенте, что именно этот аддон рисует и всплывающую подсказку умения на хотбаре, и панель
  описания в **Actions & Traits** — это один и тот же аддон, второй добавлять не нужно (отдельный
  `Tooltip` в тестах оказался лишним источником и дублировал текст поверх экрана без рамки окна).
  Плагин ищет текстовую ноду с описанием не по жёстко зашитому ID, а по совпадению текста, поэтому
  переживёт мелкие патчи игры — но имя аддона стоит один раз проверить (см. ниже), это может
  отличаться между версиями/платформами игры (ПК/консоль, регион).
